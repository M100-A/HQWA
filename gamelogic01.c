...
int gamelogic01_event_scene002 (char cSubFlag)
{
    ...
    if (gpstructMapPosDynamicCurrent[1].sVisitCount == 3)
    {
        ...
        iRet = mdialog_addToDialogWithSrchRep ("#!00# arrives back at the T-intersection where she had set-off the trap that sealed her inside this dungeon. #!00# thinks to herself if she could find what she set off she might be able to undo it. She spends a minute walking around the T-intersection trying to find where the switch is, but has no luck. She gives up, decides that she is just wasting time here, and should go elsewhere.\n\n");
        if (iRet != 0) return iRet;
    }
...
int gamelogic01_event_scene007 (char cSubFlag)
{
    ...
    // if these are not reset, then the game has problems with reloading the Link and Pos data when the memory isn't allocated for it.
    // this is only a problem for the CGI-BIN version of the program when reloading the end scene.
    // it is triggered because of the reset of gstructActStateGlobal.cEnteredDungeon above.
